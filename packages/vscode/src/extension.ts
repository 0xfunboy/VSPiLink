...
class ExtensionController {
    ...
    const accessChoices = [
      {
        ...
        description: "Read-only workspace access, coordination, and outbound network access",
        value: ["coordination:read", "coordination:write", "workspace:read", "network:outbound"],
      },
      {
        label: "Edit workspace",
        description: "Can read and write files; process execution stays disabled",
        value: ["coordination:read", "coordination:write", "workspace:read", "workspace:write", "network:outbound"],
      },
      ...(snapshot.unsafeFullAccess ? [{
        label: "Edit + run processes",
        description: "Available only because PI_UNSAFE_FULL_ACCESS is enabled",
        value: ["coordination:read", "coordination:write", "workspace:read", "workspace:write", "process:execute", "network:outbound"],
      }] : []),
    ];
  ...
  private async serveConfigured(): Promise<void> {
    ...
    await this.runCli(["serve"], "Local · secure");
  }
